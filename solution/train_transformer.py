...
def require_torch():
    try:
        ...
        raise ModuleNotFoundError(
            "PyTorch is required for Transformer training. Install it with `pip install -r requirements.txt`."
        ) from exc
...
def train_one_run(args, horizon, run_id, df):
    ...
    out_dir = Path(args.output_dir) / f"transformer_h{horizon}" / f"run_{run_id + 1}"
    ...
    for plot_idx in plot_indices:
        save_prediction_plot(
            ...
            pred[plot_idx],
            metadata,
            title=f"Transformer horizon={horizon}, run={run_id + 1}, sample={plot_idx + 1}",
        )
    save_prediction_plot(
        ...
        pred[-1],
        metadata,
        title=f"Transformer horizon={horizon}, run={run_id + 1}, last test sample",
    )
    return metrics
...
def run_experiments(args):
    ...
    for horizon in args.horizons:
        ...
        for run_id in range(args.runs):
            ...
            print(
                f"Transformer horizon={horizon} run={run_id + 1}: "
        ...
        save_json(Path(args.output_dir) / f"transformer_h{horizon}" / "summary.json", summary)
        print(
            f"Transformer horizon={horizon} summary: "
    ...
    metrics_df.to_csv(Path(args.output_dir) / "transformer_metrics.csv", index=False)


def parse_args():
    parser = argparse.ArgumentParser(description="Question 2: Transformer forecasting.")
